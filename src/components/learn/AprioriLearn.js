    ...
    return (
      <div>
        ...
        <p>
          ...
          transaction is sorted by it’s most frequent item, and infrequent items
          are removed
        </p>
        ...
        <p>
          ...
          item, and keeping a count of those, and then counting them to find the
          frequent itemsets. Example is shown below
        </p>
        <Image src={Sorted} />
        <h2>When to use FPTree vs Apriori</h2>
        <p>
          Although FPTree and Apriori come out to the same result, the one issue
          with Apriori is that it must make multiple calls to the database to
          obtain the transactions and process them. FPTree growth only requires
          two total calls to the database, and after that, our FPTree is used to
